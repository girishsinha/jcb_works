...
const ViewMachines = () => {
  ...
  const handleDelete = async () => {
    if (!deleteReason.trim()) return alert('Please enter a reason for deletion.');
    try {
  ...
  return (
    <div className="p-4 bg-gray-100 dark:bg-gray-900 min-h-screen">
      <div className="max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-4">
        <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">Registered Machines</h2>
          ...
          <table className="min-w-full text-sm text-left">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y dark:divide-gray-700">
              {currentRecords.length === 0 ? (
                <tr>
                  <td colSpan="5" className="text-center text-gray-500 py-6">No machines found</td>
                </tr>
              ) : (
                currentRecords.map((machine) => (
                  <tr key={machine.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} className="px-3 py-1 border rounded">
            Next
          </button>
        </div>
      </div>
          ...
          <div className="flex justify-center items-start min-h-screen pt-10 px-2">
            <div className="bg-white dark:bg-gray-900 max-w-5xl w-full rounded shadow-lg p-6 relative">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-purple-700 dark:text-purple-300">Machine Details</h2>
                <button onClick={() => setShowModal(false)} className="text-gray-600 dark:text-gray-300 hover:text-red-500">✖</button>
              </div>
              ...
              <div className="mt-6 flex justify-between gap-4">
                <button
                  ...
                  className={`${editMode ? 'bg-green-600' : 'bg-yellow-500'} text-white px-6 py-2 rounded-md`}
                >
      ...
      {showDeletePopup && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-md w-full">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Reason for Deletion</h3>
            <textarea
              value={deleteReason}
