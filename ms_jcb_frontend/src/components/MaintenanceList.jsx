...
export default function MaintenanceList() {
  const [records, setRecords] = useState([]);
  const [machines, setMachines] = useState([]);
  const [filters, setFilters] = useState({ machine: '', month: '', year: new Date().getFullYear() });
  const [editing, setEditing] = useState(null);
  ...
  const renderTableRows = (machineRecords) => (
    machineRecords.map((r) => (
      <tr key={r.id} className="border">
        <td className="p-2">{machines.find(m => m.id === r.machine)?.machine_number || r.machine}</td>
        <td className="p-2">{r.maintenance_type}</td>
        <td className="p-2">{r.part_repaired}</td>
        ...
        <td className="p-2 text-center">
          {r.bill_file ? (
            <a href={r.bill_file} target="_blank" rel="noreferrer" title="View Bill" className="text-blue-600 text-lg">
              👁️
            </a>
          ) : (
            <span title="No Bill Uploaded" className="text-red-500 text-lg">❌</span>
          )}
        </td>
    ...
    <div className="mt-6">
      ...
      {records.length > 0 ? (
        filters.machine ? (
          ...
          Object.entries(groupedRecords()).map(([machineId, machineRecords]) => (
            <div key={machineId} className="mb-8">
              <h3 className="font-bold text-lg text-blue-800 mb-2">
                Machine: {machines.find(m => m.id === parseInt(machineId))?.machine_number || `ID ${machineId}`}
              </h3>
