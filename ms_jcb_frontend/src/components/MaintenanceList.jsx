...
export default function MaintenanceList() {
  ...
  useEffect(() => { getMachines().then(res => setMachines(res.data)); }, []);
  useEffect(() => { loadRecords(); }, [filters]);

  return (
    <div className="p-6 space-y-6 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-200">
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-700">
              <th className="px-6 py-3 text-left text-sm font-medium">Machine</th>
              <th className="px-6 py-3 text-left text-sm font-medium">Type</th>
              <th className="px-6 py-3 text-left text-sm font-medium">Date</th>
              <th className="px-6 py-3 text-right text-sm font-medium">Cost</th>
              <th className="px-6 py-3 text-center text-sm font-medium">Bill</th>
              <th className="px-6 py-3 text-center text-sm font-medium">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {records.map(r => (
              <tr key={r.id} className="hover:bg-purple-50 dark:hover:bg-gray-700">
                <td className="px-6 py-4 text-sm">
                  {machines.find(m => m.id === r.machine)?.machine_number || r.machine}
                </td>
                <td className="px-6 py-4 text-sm text-left">{r.maintenance_type}</td>
  ...
  <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center px-4">
    <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div className="space-y-4 text-sm">
        {[
          ['Machine', machines.find(m => m.id === viewing.machine)?.machine_number || '—'],
          ...
          >
            ...
            <span className="text-gray-800 dark:text-gray-100 text-left w-1/2">{value}</span>
          </div>
        ))}
      </div>
