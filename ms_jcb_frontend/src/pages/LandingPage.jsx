...
// Sidebar dropdown
const DropdownNav = ({ id, title, icon, items, openDropdown, setOpenDropdown }) => {
      ...
      <div className="flex h-screen bg-gray-100 dark:bg-gray-900 text-black dark:text-white transition-all">
        ...
        <aside className="w-64 bg-white dark:bg-gray-800 border-r flex flex-col">
          ...
          <div className="flex flex-col flex-grow p-4 space-y-2">
            ...
            {authorityLevel === 1 && (
              <DropdownNav
                ...
                icon={<Users size={20} />}
                items={[
                  { name: "User Registration", action: () => setActiveSection("user-registration") },
                  { name: "User Activity", action: () => setActiveSection("user-activity") },
                ]}
              />
            )}
            ...
            {authorityLevel !== 3 && (
              <DropdownNav
                ...
                icon={<Wrench size={20} />}
                items={[
                  { name: "Add Machine", action: () => setActiveSection("add-machine") },
                  { name: "View Machines", action: () => setActiveSection("view-machine") },
                ]}
              />
            )}
            ...
            <DropdownNav
              ...
              items={[
                { name: "Work Entry Form", action: () => setActiveSection("work-entry") },
                { name: "Work Records", action: () => setActiveSection("work-records") },
              ]}
            />
            ...
            <DropdownNav
              ...
              icon={<Wrench size={20} />}
              items={[
                { name: "Maintenance Form", action: () => setActiveSection("maintenance-form") },
                { name: "Maintenance Record", action: () => setActiveSection("maintenance-list") },
              ]}
            />
            ...
            <DropdownNav
              ...
              items={[
                ...(authorityLevel !== 3
              ? [
                  { name: "Operator Registration", action: () => setActiveSection("operator-form") },
                  { name: "View Operators", action: () => setActiveSection("operator-page") },
                  
                ]
              : []),
            { name: "Salary Tracker", action: () => setActiveSection("salary-tracker") },
                      ]}
            />
          </div>
          ...
          <div className="mt-auto px-4 py-3 flex flex-col items-center border-t dark:border-gray-700">
            ...
            />
            <p className="text-sm font-medium text-center mb-3">{displayName}</p>
        ...
        {/* Main content */}
        <main className="flex-1 overflow-y-auto p-6 space-y-16 scroll-smooth">
          {activeSection === "home" && (
            <section
              id="home"
